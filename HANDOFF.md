...
|---|---|---|---|---|
| `batch_011` — town & shop NPCs, DATA 647–706 | `tl/script-011` @ `a1581f3` | **[#34](https://github.com/ehekatlOf/RiotStarsTranslation/pull/34)** | **2** | ✅ **MERGED — round 2, squash `edd6d2d`. Integration commit: see below.** Round 1’s single finding is FIXED and re-measured independently: D689 now reads `Ｔｈａｎｋｓ　ｔｏ　ｙｏｕ，　ｏｕｒ` (18) / `ｓｈｏｐ　ｉｓ　ｂｏｏｍｉｎｇ，　ｔｏｏ！！` (22), **+5 chars = +10 bytes**, `{FFFE}` 11→11, 36 non-`{FFFE}` tags identical, row parity 16/16, both rows ≤ 23 columns. **All three additive も in the unit now carry** (own sweep of all twelve も; the other nine are でも / 〜てもらう / もの / とっても / 他にも). **Figures: 60 lines / 60 instances, +2,474 B.** Banks 14 **12,681** · 15 **12,587** · 16 **10,953** · 17 **13,135** · 18 **10,333** free — reproduced from the reviewer’s own baseline, not the PR’s. ⚠️ **The PR body was never refreshed after the rework and still says bank 16 → 10,963 and “one commit”; the true figure is 10,953.** Banks 5 / 40 / 41 **byte-for-byte untouched** at 1,635 / 75 / 353. Gated **with no working tree**: `merge-tree --write-tree` → `452615e`, two `git archive` extractions (candidate + bare base), provenance by `cmp` + sha256 (`e1889a72…`) against `git show a1581f3:…`, and `diff -rq` showing exactly one added file. Gate 7 this round: **2,322 keys / 131 occurring / 0 failures**, controls both ways; §AG6 mirror: 44 source runs, 17 with incumbents, and **`機会` and `不思議` are unrecorded incumbents the unit conforms to**. Gate 6 coverage stated: 60 keys vs 461 shipped across 10 files, 0 collisions; D691 still byte-identical to its two `batch_009` twins. Round 1’s four settled flags re-verified present, **not reopened**. |
...
| `batch_012` — main plot + casino, DATA 355–415 | `tl/script-012` @ `b7cb51e` | **[#35](https://github.com/ehekatlOf/RiotStarsTranslation/pull/35)** | **2** | ⚠️ **ROUND 1 → CHANGES, 3 findings (1 file change, 2 PR-body only). Rework sent 2026-09-10.** All 8 gates green, **every PR figure reproduced independently**. Gate 7: **1,072 keys / 119 occurring / 303 pairs / 0 genuine failures** (38 apparent, all adjudicated by reading). Gated **no working tree** — `merge-tree` → `2e42c97`, `git archive`, provenance by `cmp` + sha256. **The file finding: a FALSE IMPOSSIBILITY.** Flag 4b claimed D390 could not carry `〜をいいことに` (*taking advantage of the fact that*, pointedly opportunistic) — but bank 1 has **26,203 free** and the reviewer's own DP wrap found a **4-row / 88-column** rendering at ≤23, with three further orderings also reaching 4 rows. The idiom is dropped **in the sentence that characterises the usurper**, so it goes back — the same standard #34 was returned under for a smaller loss. Also: the `いやはや、` reach figure is wrong (4 rows / 2 source strings, not "3 — exhausted"), and `ベルナール教会` is rendered first here and **missing from Glossary additions**, so §4.6 requires promotion. ⚠️ **Reviewer corrected THREE of its own instrumentation bugs before asserting anything** (splitter ignoring `{FC30}`/`{FC50}`/`{FC51}`/`{FFFF}`; ellipsis counting missing §3.1's `・・・。`=4; a false `Ｄｅａｒ　ｍｅ` collision) — the PR's figures were right and its own tooling was wrong. Cross-unit **discharged**: `batch_013` has **0** `ホッジス`, so no disagreement; `『極上のワイン』` row **stays live** for #36. |
...
**Reviews: #34 → CHANGES (round 2 pushed at `a1581f3`, awaiting re-review); #35 → CHANGES (rework
in flight); #36 not yet reviewed.** ⚠️ **A rework message must NOT tell a translator to
